...
\begin{problem}[Mandatory; No LLMs; 20 pts]
    Prove the soundness of contraction-free sequent calculus, i.e., if $\G \cseq A$, then $\G \seq A$.
\end{problem}

\begin{problem}[Mandatory; No LLMs; 30 pts]
Complete the proof of completeness of contraction-free sequent calculus, i.e.,
i.e., if $\G \seq A$, then $\G \cseq A$.
You only need to show the case when the derivation of $\G \seq A$ ends in $\limpl L$ rule.
\end{problem}

\begin{problem}[Mandatory; No LLMs; 50 pts]
Define your judgments and write out \textbf{all the rules}.

\textbf{Hint:} It would help to define at least three judgments:
right inversion, left inversion, and choice.
\end{problem}

\begin{problem}[Optional; LLMs ok; 20 pts]
Prove identity expansion for the inversion calculus. Concretely, show that $\epsilon \semi A \irseq A$
holds for any arbitrary proposition $A$ in the inversion calculus.
\end{problem}

\begin{problem}[Optional; LLMs ok; 10 pts]
Does $\epsilon \semi A \ilseq A$ hold for any arbitrary proposition $A$ in the inversion calculus?
Briefly justify your answer. If it does not hold, give a counterexample $A$ for which this statement
does not hold.
\end{problem}

\begin{problem}[Optional; LLMs ok; 10 pts]
Does $\epsilon \semi A \icseq A$ hold for any arbitrary proposition $A$ in the inversion calculus?
Briefly justify your answer. If it does not hold, give a counterexample $A$ for which this statement
does not hold.
\end{problem}
